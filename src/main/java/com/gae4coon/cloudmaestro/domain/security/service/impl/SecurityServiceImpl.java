...
public class SecurityServiceImpl implements SecurityService {
    ...
    public HashMap<String, Object> security(RequireDTO requireDTO, Map<String, Object> responseArray) {
        ...
        diagramDTOService = new DiagramDTOService(responseArray);
    ...
    private void setIAM() {
        if (diagramDTOService.isNodeDataContains("Identity and Access Management")) return;
        diagramDTOService.createServiceGroup();
    ...
    private void setSecurityHub() {
        if (diagramDTOService.isNodeDataContains("Security Hub")) return;
        diagramDTOService.createServiceGroup();
    ...
    private void setGuardDuty() {
        if (diagramDTOService.isNodeDataContains("GuardDuty")) return;
        diagramDTOService.createServiceGroup();
    ...
    private void setDetective() {
        if (diagramDTOService.isNodeDataContains("Detective")) return;
        diagramDTOService.createServiceGroup();
    ...
    private void setNetworkFW() {
        ...
        if (!diagramDTOService.isNodeDataContains("Network Firewall")) {
    ...
    private void setShield() {
        if (diagramDTOService.isNodeDataContains("Shield")) return;
        diagramDTOService.createServiceGroup();
    ...
    private void setGlobalWAF() {
        ...
        if (diagramDTOService.isNodeDataContains("CloudFront")) {
            containResource.add("CloudFront");
        }
        if (diagramDTOService.isNodeDataContains("API Gateway")) {
            containResource.add("API Gateway");
        }
        if (containResource.isEmpty()) return;

        // 다이어그램에 WAF 없으면 예외처리
        if (!diagramDTOService.isNodeDataContains("AWS_WAF")) {
    ...
    private void setSecretsManager() {
        if (diagramDTOService.isNodeDataContains("Secrets Manager")) return;
        diagramDTOService.createServiceGroup();
    ...
    private void setZoneWAF(String zoneName) {
        ...
        List<NodeData> ALBList = diagramDTOService.getNodeListByText("Application Load Balancer(ALB)");
        ...
        for (NodeData alb : ALBList) {
            // ALB가 아니면 continue
            LinkData ALBLink = diagramDTOService.getLinkDataByTo(alb.getKey());
            if (ALBLink == null) continue;

            NodeData ALBToNode = diagramDTOService.getNodeDataByKey(ALBLink.getTo());
            GroupData ALBToGroup = diagramDTOService.getGroupDataByKey(ALBLink.getTo());
